...
export async function writeLastApply(
  planOrJournal: DevLinkPlan | LastApplyJournal,
  executed?: LinkAction[],
  manifestPatches?: ManifestPatch[],
  backupDir?: string
): Promise<void> {
  // Legacy usage: writeLastApply(journal)
  if (executed === undefined) {
    const journal = planOrJournal as LastApplyJournal;
    const journalPath = `${journal.rootDir}/.kb/devlink/last-apply.json`;
    await writeJson(journalPath, journal);
    logger.debug("Last-apply journal written (legacy)", {
      path: journalPath,
      actions: journal.actions?.length ?? 0,
  ...
  const plan = planOrJournal as DevLinkPlan;
  const executedActions = executed ?? [];
