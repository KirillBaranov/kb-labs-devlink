...
import { readJson, exists, writeJson } from '@devlink/shared/utils/fs';
...
export async function undoLastApply(
  ...
  if (!journal.backupDir || !(await exists(journal.backupDir))) {
    if (opts.dryRun) {
      logger.warn("Backup directory missing during dry-run undo", { backupDir: journal.backupDir });
      return;
    }
    throw new Error(
...
export async function undoLastOperation(
  ...
  if (!operation) {
    throw new Error(
      "No operations to undo. All recent operations are already undone."
    );
  }
