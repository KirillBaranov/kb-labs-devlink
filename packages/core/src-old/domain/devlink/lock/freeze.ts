...
import { writeJson, readJson, exists } from '@devlink/shared/utils/fs';
import type { DevLinkPlan } from '../types';
import { logger } from '@devlink/shared/utils/logger';
import { resolveInstalledVersionNear } from '@devlink/infra/discovery/discovery';
...
export async function freezeToLockMerged(
  ...
  for (const [consumerName, actions] of actionsByConsumer) {
    ...
    const consumerDir = consumerMeta?.dir ?? resolvedRootDir;
  ...
  if (prune) {
    for (const [consumerName, consumer] of Object.entries(lockFile.consumers)) {
      if (actionsByConsumer.has(consumerName)) {
        continue;
      }

      const fallbackDir = plan.index.packages[consumerName]?.dir ?? resolvedRootDir;
      const depsToRemove: string[] = [];

      for (const dep of Object.keys(consumer.deps)) {
        const isDeclared = await isDeclaredInManifest(dep, fallbackDir);
        if (!isDeclared) {
          depsToRemove.push(dep);
        }
      }
