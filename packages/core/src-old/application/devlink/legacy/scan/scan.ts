...
import { discover } from '@devlink/infra/discovery/discovery';
import { logger } from '@devlink/shared/utils/logger';
import type { DepEdge, DevlinkState, PackageJson } from '@devlink/shared/types';
...
export async function scanPackages(opts: ScanOptions): Promise<{
  ...
  const { rootDir, container } = opts;
  let roots = ((opts as any).roots as string[] | undefined)?.filter(Boolean);
  ...
  const state = await discover(roots?.length ? { roots, forceContainer: container } : { forceContainer: container });
