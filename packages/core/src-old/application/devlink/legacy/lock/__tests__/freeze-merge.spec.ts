...
import { readJson, writeJson, exists } from '@devlink/shared/utils/fs';
...
describe("freezeToLockMerged", () => {
  ...
  it("should merge with existing lock entries", async () => {
    ...
    expect(lock.consumers["<root>"]?.deps?.["old-pkg"]).toBeDefined();
    expect(lock.consumers["pkg-a"]?.deps?.["pkg-b"]).toBeDefined();
  });
  ...
  it("should replace all entries when replace=true", async () => {
    ...
    const lock = await readJson<any>(lockPath);
    expect(lock.consumers["<root>"]?.deps?.["old-pkg"]).toBeUndefined();
    expect(lock.consumers["pkg-a"]?.deps?.["pkg-b"]).toBeDefined();
  });
  ...
  it("should detect workspace source for same-workspace packages", async () => {
    ...
    const lock = await readJson<any>(lockPath);
    expect(lock.consumers["pkg-a"]?.deps?.["pkg-b"].source).toBe("workspace");
  });
  ...
  it("should detect github source for github: prefix", async () => {
    ...
    const lock = await readJson<any>(lockPath);
    expect(lock.consumers["pkg-a"]?.deps?.["github:user/repo"].source).toBe("github");
  });
});
