...
import { discoverArtifacts, determineMode } from '@devlink/application/devlink/legacy/status';
...
import { DevlinkFreezeCommandInputSchema } from '@kb-labs/devlink-contracts/schema';
import { parseCommandFlags } from './utils';

type FreezeCommandFlags = z.infer<typeof DevlinkFreezeCommandInputSchema>;

export const run: CommandModule<FreezeCommandFlags>['run'] = async (ctx, _argv, rawFlags) => {
  const startTime = Date.now();
  const jsonMode = !!(rawFlags as FreezeCommandFlags | undefined)?.json;
  const flags = parseCommandFlags(DevlinkFreezeCommandInputSchema, rawFlags, {
    ctx,
    command: 'devlink freeze',
    jsonMode,
  });
