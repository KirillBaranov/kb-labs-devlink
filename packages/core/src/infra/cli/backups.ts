...
import { DevlinkBackupsCommandInputSchema } from '@kb-labs/devlink-contracts/schema';
import { parseCommandFlags } from './utils';

type BackupsCommandFlags = z.infer<typeof DevlinkBackupsCommandInputSchema>;

export const run: CommandModule<BackupsCommandFlags>['run'] = async (ctx, _argv, rawFlags): Promise<number | void> => {
  const startTime = Date.now();
  const jsonMode = !!(rawFlags as BackupsCommandFlags | undefined)?.json;
  const flags = parseCommandFlags(DevlinkBackupsCommandInputSchema, rawFlags, {
    ctx,
    command: 'devlink backups',
    jsonMode,
  });
