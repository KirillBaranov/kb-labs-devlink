import { runCommand } from '@kb-labs/devlink-adapters/process';
...
import { saveState } from '@kb-labs/devlink-adapters/state';
import type { DevlinkState } from '../types';
import { writeLastApply } from '../models/journal/last-apply';
import { discover } from '../discovery/discovery';
import { clean } from '@kb-labs/devlink-adapters/maintenance';
...
export async function applyPlan(plan: DevLinkPlan, opts: ApplyOptions = {}): Promise<ApplyResult> {
  ...
  if (opts.dryRun) {
    ...
    const actionsCopy = plan.actions ? [...plan.actions] : [];
    return {
      ok: true,
      executed: actionsCopy,
      skipped: [],
      errors: [],
      needsInstall: false,
      manifestPatches: [],
